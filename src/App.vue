...
/* 记住最近浏览的论文 id：点站名返回首页时它是回程动画的起点卡片。
   immediate 覆盖直接以论文链接打开页面的情况 */
watch(
  current,
  (m) => {
    if (m) paperVt.lastId = m.id
  },
  { immediate: true },
)
...
<template>
  ...
  <!-- 不用 router-link：fallthrough 的 @click 排在其内部 navigate 之后，
       站名返回的过渡同样会失效（同 HomeView 卡片） -->
  <a href="#/" class="brand" :class="{ collapsed: titleMode }" @click="onBrandClick">
      <span>论文双语对照</span>
    </a>
